...
class CardGenerator:
    ...
    def get_cards_by_rarity(
        ...
        Returns:
            Dictionary with card types as keys
            and lists of matching cards as values
        """
    ...
    def list_available_cards(self) -> None:
        ...
        for creature in self._creatures:
            print(
                f"  - {creature['name']} "
                f"({creature['cost']} mana, {creature['rarity']})"
            )
        ...
        for spell in self._spells:
            print(
                f"  - {spell['name']} "
                f"({spell['cost']} mana, {spell['rarity']})"
            )
        ...
        for artifact in self._artifacts:
            print(
                f"  - {artifact['name']} "
                f"({artifact['cost']} mana, {artifact['rarity']})"
            )
...
def main() -> None:
    ...
    random_deck = [card["name"] for card in generator.generate_random_deck(5)]
    print(f"\nRandom Deck (5 cards): {random_deck}")
